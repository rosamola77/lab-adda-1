...
public class Tokenizer {
	...
	public Token currentToken() {
		Preconditions.checkState(index>0, String.format("No hay token actual todav�a %d",index));
		return 	tokens.get(index-1);	
	}

	public Token nextToken(){
		Preconditions.checkState(hasMoreTokens(), 
			String.format("Cadena acabada inesperadamente en la posici�n %d,%s",index(),suffix()));
		...
		if (Character.isSpaceChar(c)) {
			...
			if (match) {
				...
				Preconditions.checkState(false, 
						String.format("Caracter %c no reconocido en la posici�n %d",c,start));
			}			
		}		
		int oldStart = start;
		start = start + inc;
	...
	public void error() {
		Preconditions.checkState(false, 
				String.format("Token %s no reconocido en la posici�n %d, %s",
